...
type Book struct {
	...
	QuantityOrigin int    `json:"quantity_origin"`
}
...
type RepositoryBook interface {
	...
	GetBookByUserNameBook(ctx context.Context, nameBook string) (*Book, error)

	GetCountBookByUserNameBook(ctx context.Context, nameBook string) (*Book, error)

	GetAuthorBookByBook(ctx context.Context, nameBook string) (*Author, error)

	GetBookByIUseOrder(ctx context.Context, id int64) (*Book, error)
