...
func (u *CollectionBook) GetBookByUserNameBook(ctx context.Context, nameBook string) (*domain.Book, error) {
	var book *domain.Book

	result := u.book.Where("title ILIKE ? AND is_active = true AND quantity > 0", "%"+nameBook+"%").First(&book)
	if errors.Is(result.Error, gorm.ErrRecordNotFound) {
		return nil, nil
	}
	return book, result.Error
}
func (u *CollectionBook) GetCountBookByUserNameBook(ctx context.Context, nameBook string) (*domain.Book, error) {
	var books []*domain.Book
	u.book.Where("title ILIKE ? AND is_active = true", "%"+nameBook+"%").Find(&books)
	countBook := len(books)
	...
	randomIndex := rand.Intn(countBook)

	return books[randomIndex], nil
}

func (u *CollectionBook) GetAuthorBookByBook(ctx context.Context, nameBook string) (*domain.Author, error) {
	var book *domain.Book
	var author *domain.Author
	u.book.Where("title ILIKE ? AND is_active = true", "%"+nameBook+"%").First(&book)
	result := u.book.Where("name = ?", book.AuthorName).First(&author)
...
func (c *CollectionBook) GetBookByIUseOrder(ctx context.Context, id int64) (*domain.Book, error) {
	var book *domain.Book
	result := c.book.Where("id = ?", id).First(&book)
	if errors.Is(result.Error, gorm.ErrRecordNotFound) {
		return nil, nil
...
func (u *CollectionBook) GetListBookByTypeBookBotUseBot(ctx context.Context, nameBook string) ([]*domain.Book, error) {
	var books = make([]*domain.Book, 0)

	result := u.book.Where("genre ILIKE ? AND is_active = true AND quantity > 0", "%"+nameBook+"%").Find(&books)
