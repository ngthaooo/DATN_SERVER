...
func (c *CollectionTypeBook) GetListTypeBookByName(ctx context.Context, name string) ([]*domain.TypeBooks, error) {
	var genres []*domain.TypeBooks
	// Sử dụng ILIKE với cú pháp đúng và thêm ký tự đại diện để tìm kiếm
	result := c.db.Where("name ILIKE ?", "%"+name+"%").Where("is_active = true").Find(&genres)
	if result.Error != nil {
		return nil, result.Error
	}
	return genres, nil
